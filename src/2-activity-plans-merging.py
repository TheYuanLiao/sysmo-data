...
class PlansProcessor:
    ...
    def load_original_plans(self):
        ...
        self.df_plan = pd.read_pickle(os.path.join(ROOT_dir, 'dbs/df_act_plan.pkl'))
    ...
    def load_input_plans(self, region=None, test=False):
        input_file = os.path.join(ROOT_dir, f'dbs/scenarios/{region}/plans_cars.xml.gz')
    ...
    def load_output_plans(self, region=None):
        output_file = os.path.join(ROOT_dir, f'dbs/scenarios/{region}/output/output_experienced_plans.xml.gz')
...
if __name__ == '__main__':
    ...
    for region in ('vast13', 'vast14', 'vast17'):
        ...
        df.to_csv(os.path.join(ROOT_dir, f'dbs/output/plans_{region}.csv.gz'), compression='gzip', index=False)